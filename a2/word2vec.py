...
def naiveSoftmaxLossAndGradient(
    ...
    UVc = np.matmul(outsideVectors, centerWordVec)
    y_hat = softmax(UVc)
    loss = -np.log(y_hat[outsideWordIdx])
    y = np.zeros(UVc.shape)
    y[outsideWordIdx] = 1
...
def negSamplingLossAndGradient(
    ...
    subsetU = outsideVectors[indices,:]
    UVc = np.matmul(subsetU, centerWordVec)
    sigmoidsInput = -UVc
    sigmoidsInput[0] *= -1
    sigmoids = sigmoid(sigmoidsInput)
    logs = np.log(sigmoids)
    loss = -logs[0] - np.sum(logs[1:])
    b = (-outsideVectors[negSampleWordIndices,:].T * (1 - sigmoids[1:])).T
    a = np.sum(b, axis=0)
    items, occurrences = np.unique(negSampleWordIndices, return_counts=True)
    coeff = np.zeros(outsideVectors.shape[0])
    for i, o in zip(items, occurrences):
        coeff[i] = o
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradCenterVec = (sigmoids[0] - 1) * outsideVectors[outsideWordIdx] - a
    tiled = np.tile(centerWordVec, (K, 1))
    gradOutsideVecs[negSampleWordIndices,:] = (1 - sigmoids[1:]).reshape(-1, 1) * tiled
    gradOutsideVecs = gradOutsideVecs * coeff.reshape(-1, 1)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerIndex = word2Ind[currentCenterWord]
    centerVec = centerWordVectors[centerIndex, :]

    for word in outsideWords:
        outsideIndex = word2Ind[word]
        curLoss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerVec, outsideIndex, outsideVectors, dataset)
        loss += curLoss
        gradCenterVecs[centerIndex, :] += gradCenterVec
